...
public class Accessor extends Resource
{
	...
	public <TYPE> TYPE[] executeCall(Trace parent, String traceCategoryOverride, Class<TYPE> type, String sql, Object... parameters) throws Throwable
	{
		return executeCall(parent, traceCategoryOverride, type, parameters, sql);
	}

	public <TYPE> TYPE[] executeCall(Trace parent, String traceCategoryOverride, Class<TYPE> type, Object[] parameters, String sql) throws Throwable
	{
		if (traceCategoryOverride == null)
		{
			traceCategoryOverride = sql;
			...
			try
			{
				try (CallableStatement statement = this.connection.prepareCall(sql))
				{
					...
					if (statement.execute() == false)
					{
                        ...
                        return new CallRowSetResult<RETURN_TYPE>(returnValue,outValues,null);
					}
					try (ResultSet resultSet = statement.getResultSet())
					{
						...
						return new CallRowSetResult<RETURN_TYPE>(returnValue,outValues,results);
					}
				}
			}
			catch (SQLException ex)
			...
			try
			{
				try (CallableStatement statement = this.connection.prepareCall(sql))
				{
					...
					if (statement.execute() == false)
					{
                        ...
                        return new CallResult<ROW_TYPE,RETURN_TYPE>(returnValue,outValues,null);
					}
					try (ResultSet resultSet = statement.getResultSet())
					{
						...
						return new CallResult<ROW_TYPE,RETURN_TYPE>(returnValue,outValues,results);
					}
				}
			}
			catch (SQLException ex)
