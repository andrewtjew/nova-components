...
public class HtmlWriter
{
	...
	/*
	public HtmlWriter getJSONButtonAction(String id,String text,String url,String resultId,boolean async,String...ids)
	{
		tag("button",text,new Attribute("id",id));
		this.sb.append("<script type='text/javascript'>$('#"+id+"').click(function(){$.ajax({type:'get',url:'"+url+"',dataType:'json',data:{");
		boolean commaNeeded=false;
		for (String item:ids)
		{
			if (commaNeeded==false)
			{
				sb.append("'"+item+"':$('#"+item+"').val()");
			}
			else
			{
				sb.append(",'"+item+"':$('#"+item+"').val()");
...
//		this.sb.append("},async:"+async+",error:function(data){alert('error'+data.content);},success:function(data){alert(data);$('#"+resultId+"').html(data.content);} });});</script>");
		this.sb.append("},async:"+async+",success:function(data){$.each(data,function(key,value){$('#'+key).html(value);})} });});</script>");
